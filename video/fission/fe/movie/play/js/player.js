var vfed = {
    'player': {
        ...
        'eplayer': function(auto, live, trys, seek, take, urls, jump, logo, pics) {
                    ...
                    customType: {
                        'customHls': function(video, player) {
                            ...
                            engine.on('peerId', function(peerId) {
                                ...
                                $('.load').text('鍔犺浇0MB 鍏变韩0MB 鍔犻€�0MB');
                                $('.peer').text('P2P宸插紑鍚�');
                                $('.line').text('鍦ㄧ嚎1NP');
                            });
                            engine.on('peers', function(peers) {
                                $('.line').text('鍦ㄧ嚎' + (peers.length + 1) + 'NP');
                                $('.peer').text('P2P宸插紑鍚�');
                            });
                            engine.on('stats', function(data) {
                                $('.load').text('鍔犺浇' + (data.totalHTTPDownloaded / 1024).toFixed(2) + 'MB 鍏变韩' + (data.totalP2PUploaded / 1024).toFixed(2) + 'MB 鍔犻€�' + (data.totalP2PDownloaded / 1024).toFixed(2) + 'MB');
            ...
            player.on('play', function() {
                $("#nm_p").removeClass("dplayer-hide-controller")
                $('.play').hide();
            });
            $('.play').click(function() {
    ...
    'cookie': {
        ...
        'put': function(urls) {
            var cookie = urls.replace(/[^a-zA-Z0-9]+/ig, '');
            var cookie = cookie.substring(cookie.length - 200);
            return cookie;
        }
    }
};
